...
class Climbs extends Table {
  ...
  IntColumn get wallTypeId =>
      integer().nullable().references(WallTypes, #id, onDelete: KeyAction.setNull)();
...
class AppDatabase extends _$AppDatabase {
  ...
  MigrationStrategy get migration => MigrationStrategy(
        ...
        onUpgrade: (m, from, to) async {
          if (from < 2) {
            ...
            final withPhoto = await (select(climbs)
                  ..where((c) => c.photoPath.isNotNull()))
                .get();
  ...
  Stream<List<Gym>> watchGyms() =>
      (select(gyms)..orderBy([(g) => OrderingTerm(expression: g.name)])).watch();
  ...
  Stream<List<WallType>> watchWallTypes() =>
      (select(wallTypes)..orderBy([(w) => OrderingTerm(expression: w.id)]))
          .watch();
  ...
  Future<int> insertClimbPhoto(int climbId, String path, int sortOrder) =>
      into(climbPhotos).insert(ClimbPhotosCompanion.insert(
        climbId: climbId,
        path: path,
